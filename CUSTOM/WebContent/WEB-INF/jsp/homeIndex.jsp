...
<html>
    ...
    <script src="${path}/resource/js/common.js"></script>
    	...
    	$(function(){
    			...
    			var url = "http://10.0.1.183:8088/dora/ent/toQueryList";
        		$.ajax({
        			  async : true,
     				  type: "GET",
     				  url: url,
	  	   			   		...
	  	   			   		var html = "";
 	  	   			   		for(var i=0;i<data.length;i++){
	  	   			   			...
	  	   			   			if(data[i].CHECK_TYPE==2){
	  	   			   				html += "<td>/</td><td><button type='button' class='btn btn-success' onclick=\"goChangeStatus('"+ data[i].FIAT_ID +"')\">清關</button></td></tr>";
	  	   			   			}else{
    	...
    	function goDetail(e) {
			var FIAT_ID = e;
			var urlD = "http://10.0.1.183:8088/dora/ent/toQueryDetail";
			$.ajax({
				async : true,
				  type: "GET",
				  url: urlD,
		...
		function goChangeStatus(e) {
			var FIAT_ID = e;
			var urlD = "http://10.0.1.183:8088/dora/ent/toUpdateStatus";
			$.ajax({
				async : true,
				  type: "GET",
				  url: urlD,
				  ...
				  success: function(res){
					  if(res==1){
						  alert("success");
						  window.location.href= '../ent/toKey';
